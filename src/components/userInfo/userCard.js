...
export const UserCard = ({ user, setShowLoader, isHomePage }) => {
  ...
  React.useEffect(() => {
    if (isHomePage) {
      axios(INSTAGRAM_USER_INFO(user.instagram_user_id), ApiHeaders)
        .then((userInfo) => {
        ...
        {isHomePage && (
          <CardActions className="card-action">
            ...
            <Button
              ...
              variant="contained"
              size="medium"
              style={{ backgroundColor: "#c5ae4e" }}
              onClick={() => goToRoute("insta-insights")}
            >
              FEED
            </Button>
            {
